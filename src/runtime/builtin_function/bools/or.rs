...
pub fn or(runtime: &mut Runtime, executable: &ExecutableLine)-> Result<()>{
    ...
    for param in &executable.params{
        ...
        match dt {
            ...
            _=>{
                return Err(
                    ChapError::runtime_with_msg(executable.line_number, "or function needs bool params".to_string())
                );
            }
        }
    }
    ...
    if let Some(var_name) = &executable.output_var{
        ...
        Err(
            ChapError::runtime_with_msg(executable.line_number, "or function needs output variable".to_string())
        )
    }
}