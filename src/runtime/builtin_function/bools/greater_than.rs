...
pub fn greater_than(runtime: &mut Runtime, executable: &ExecutableLine)-> Result<()>{
    ...
    let sum = greater_than_data_types(p1,p2)?;
    ...
    match &executable.output_var{
        ...
        None => Err(
            ChapError::runtime_with_msg(executable.line_number, "greater_than function needs output variable".to_string())
        ),
    }
}

pub fn greater_than_data_types(dt1: &DataType, dt2: &DataType) -> Result<DataType>{
    match (dt1, dt2) {
        ...
        _=> {
            Err(
                ChapError::runtime_with_msg(0, "greater_than function works only with numbers int and float".to_string())
            )
        }
    }
}
