...
use crate::runtime::builtin_function::utils::{param_to_datatype, param_to_datatype_mut, returns};
...
pub fn remove_at(runtime: &mut Runtime, executable: &ExecutableLine) -> Result<()> {
    ...
    let result = if let (DataType::List(list), DataType::Int(index)) = (p1, p2) {
        if let Ok(index) = usize::try_from(index) {
            ...
            return Err(ChapError::runtime_with_msg(
                executable.line_number,
                format!("{} function negative index", executable.function_name),
            ));
        }
    } else {
        return Err(ChapError::runtime_with_msg(
            executable.line_number,
            format!(
                "correct form of {0} function: <list>, <index> -> {0} -> $var",
