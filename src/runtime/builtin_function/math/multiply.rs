...
pub fn multiply(runtime: &mut Runtime, executable: &ExecutableLine)-> Result<()>{
...
fn multiply_data_types(dt1: &DataType, dt2: &DataType) -> Result<DataType>{
    match (dt1, dt2) {
        ...
        _=> {
            Err(
                ChapError::runtime_with_msg(0, "multiply function works only with numbers int and float".to_string())
            )
        }
    }
}
