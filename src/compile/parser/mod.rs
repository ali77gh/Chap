...
pub mod get_single_var;

use crate::common::chunk::Chunk;
use crate::common::errors::{ChapError, Result};
use crate::common::param::Param;
use crate::common::splitter::string_safe_split;
use crate::common::{executable::ExecutableLine, line_of_code::LineOfCode};
use chunk_detector::chunk_detector;
use get_single_var::get_single_var;
...
impl Parser {
    pub fn on_new_line(&mut self, new_line: LineOfCode) -> Result<Vec<ExecutableLine>> {
        ...
        let chunks: Vec<Result<Chunk>> = chunks
            .iter()
            .map(|x| chunk_detector(x.to_string(), ln))
            .collect();
                                    ...
                                    Some(cc1) => match cc1 {
                                        Param::Tag(_) => {
                                            if cc.len() > 1 {
                                                return Err(ChapError::syntax_with_msg(
                                                    ln,
                                                    "<tag>,<param>... does not mean anything"
                                                        .to_string(),
                                                ));
                                            }
                                            result.push(ExecutableLine::new(
                                                ln,
                                                "new_tag".to_string(),
                                                cc,
                                                None,
                                        ...
                                        _ => result.push(ExecutableLine::new(
                                            ln,
                                            "print".to_string(),
                                            cc,
                                            None,
                    ...
                    Chunk::Function(cc) => {
                        match nc {
                            ...
                            Some(nc) => match nc.clone()? {
                                ...
                                Chunk::Function(_) => result.push(ExecutableLine::new(
                                    ln,
                                    cc,
                                    vec![],
                                    Some(self.get_next_tmp()),
                                )),
                            },
                        }
                    }
...
mod tests {
    ...
    fn tag_parser() {
        ...
        let result = p
            .on_new_line(LineOfCode::new(1, " @myTag ".to_string()))
            .unwrap();
    ...
    fn print_detector_parser() {
        ...
        assert_eq!(
            ...
            &ExecutableLine::new(
                1,
                "print".to_string(),
                vec![Param::Variable("myVar".to_string())],
                None
            )
        );
        ...
        assert_eq!(
            ...
            &ExecutableLine::new(
                1,
                "print".to_string(),
                vec![Param::Value(DataType::String("hello".to_string()))],
                None
            )
        );
    }
    ...
    fn param_param_test() {
        ...
        assert_eq!(
            ...
            &ExecutableLine::new(
                1,
                "assign".to_string(),
                vec![Param::Value(DataType::Int(1))],
                Some("var".to_string())
            )
        );
    }
    ...
    fn piping_test() {
        ...
        let result = p
            .on_new_line(LineOfCode::new(
                1,
                "1, 2 -> add -> to_string -> $a".to_string(),
            ))
            .unwrap();
