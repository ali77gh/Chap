...
impl Preprocessor {
    pub fn on_new_line(&mut self, actual_line: String) -> Result<Vec<LineOfCode>> {
        ...
        Ok(lines
            .iter()
            .map(|x| LineOfCode::new(line, x.to_string()))
            .collect())
    }
                ...
                Some(end) => {
                    ...
                    match start {
                        Some(start) => {
                            ...
                            result.push(gen);
                            let rest =
                                format!("{}$TMP_{}{}", &l[..start], self.temp_vars, &l[end + 1..]);
                            self.temp_vars += 1;
...
mod tests {
    ...
    fn semicolon_split() {
        ...
        assert_eq!(
            2,
            pp.on_new_line("  command2  ;   command3  ".to_string())
                .unwrap()
                .len()
        );
        assert_eq!(
            1,
            pp.on_new_line("  command3  ;   //comment  ".to_string())
                .unwrap()
                .len()
        );
        assert_eq!(
            1,
            pp.on_new_line("  command4     //comment  ".to_string())
                .unwrap()
                .len()
        );
        assert_eq!(
            2,
            pp.on_new_line("  command5 ; ; command6     //comment  ".to_string())
                .unwrap()
                .len()
        );
    ...
    fn nest_parentheses() {
        ...
        assert_eq!(
            result.get(3).unwrap().code,
            "$TMP_1,$TMP_2->add".to_string()
        );
    }
